...
def optimization_target(cfg: DictConfig):
    ...
    with logger.start_run(run_name=run_name):
        ...
        pipeline.model.fit(inputs, outputs)
        auc_tot = roc_auc_score(outputs, pipeline.model.predict_proba(inputs)[:, 1], average=None)
        logger.log_metric(f'train auc', auc_tot)
        print(f'auc: {auc_tot}')

        pipeline.save(dir_path='/home/victore/Downloads')
        ...
        #     pipeline.model.fit(X_train_rooms, y_train_rooms)
        #     auc_room[room] = roc_auc_score(y_train_rooms, pipeline.model.predict_proba(X_train_rooms)[:, 1], average=None)
        #     logger.log_metric(f'train room {room} auc', auc_room[room])
        #
        #     print(f'testing room: {room}')
        #     auc_room_test[room] = roc_auc_score(y_test_rooms, pipeline.model.predict_proba(X_test_rooms)[:, 1], average=None)
    ...
    return auc_tot  # np.mean(auc_room_test_list)
